...
503 is semantically correct (RFC 9110 §15.6.4: "temporarily unable to handle the request … will likely be alleviated
after some delay") and is also what llama-server's own `/health` returns during model load — so monitoring tools
see a consistent "not ready yet" contract across both phases (download → model load → ready).
